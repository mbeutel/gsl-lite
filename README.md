...
Define this macro to 1 to have the non-transparent legacy version of `not_null<>::get()` return `T const &` instead of `T`. This may improve performance with types that have an expensive copy-constructor. This macro may not be defined if <b>gsl\_CONFIG\_TRANSPARENT\_NOT\_NULL</b>=1. Default is 0 for `T`.
